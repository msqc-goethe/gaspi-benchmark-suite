...
#ifdef USE_MPI3
#define PASSIVE_SEND(source, rank) MPI_Send(source, 1, MPI_LONG, rank, 0, MPI_COMM_WORLD);
#else
#ifdef USE_GASPI
#define PASSIVE_SEND(source, rank) gaspi_passive_send(segment_id, (void *)source - (void *)segment_base_ary, rank, sizeof(long long), timeout);
#endif
#endif

#ifdef USE_MPI3
#define PASSIVE_RECV(target, rank) MPI_Recv(target, 1, MPI_LONG, rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

#else
#ifdef USE_GASPI
#define PASSIVE_RECV(target, rank) gaspi_passive_receive(segment_id, (void *)target - (void *)segment_base_ary, &rank, sizeof(long long), timeout);
#endif
#endif
