...
static void fetch_score(score_matrix_t *A, index_t m, index_t n, score_t *in){
  ...
  //printf("\n fetch from Score; my rank %d; dest rank: %d ", rank, target_ep);
...
static void fetch_gap(gap_matrix_t *A, index_t m, index_t n, score_t *in){
  int target_ep = n / A->local_length;
  int local_index = n %A->local_length;
  //printf("\n fetch from Gap; my rank %d; dest rank: %d ", rank, target_ep);
...
#pragma omp parallel for
  ...
  malloc_all(sizeof(score_t), (void **)&current_main);
  malloc_all(sizeof(score_t), (void **)&current_match);
  ...
  malloc_all(sizeof(score_t), (void **)&G);
  malloc_all(sizeof(score_t), (void **)&W);
  malloc_all(sizeof(score_t), (void **)&E);
  malloc_all(sizeof(score_t), (void **)&F);
  malloc_all(sizeof(score_t), (void **)&cmp_a);
  malloc_all(sizeof(score_t), (void **)&cmp_b);
  malloc_all(sizeof(score_t), (void **)&cmp_c);
  malloc_all(sizeof(score_t), (void **)&new_score);
  malloc_all(sizeof(score_t), (void **)&next_G);
  malloc_all(sizeof(score_t), (void **)&next_F);
  malloc_all(sizeof(score_t), (void **)&next_E);
  malloc_all(sizeof(score_t), (void **)&next_match);
  malloc_all(sizeof(score_t), (void **)&next_main);
  ...
  printf("\nMain: %d Match: %d \n",*main_codon,*match_codon);
    *W = sim_matrix->similarity[*main_codon][*match_codon];
    assign_score(score_matrix,0,0,0 > *W ? (score_t*) zero_seg : W);
    *buf = -gapFirst + *W;
    assign_gap(main_gap_matrix, 0, 0, (score_t *)buf);
    assign_gap(match_gap_matrix, 0, 0, (score_t*) buf);
    //idx=1 m=0,1 n =1,0
    //fetch_from_seq(main_seq, 0, main_codon);
    ...
    *buf = -gapFirst;
    assign_gap(match_gap_matrix, 1, 0, -gapFirst > *cmp_b ? (score_t*) buf : cmp_b);
    ...
    *cmp_b = *G - gapFirst;
    *buf = -gapFirst;
    assign_gap(main_gap_matrix, 1, 1, -gapFirst > *cmp_b ? (score_t*) buf : cmp_b);
