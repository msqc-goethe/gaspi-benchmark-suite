...
if [ $gaspi_mpi_shmem = "gaspi" ] ; then
    echo "RUN GASPI"
    $exePath -m $machineFile  -n $ranks  $runfile $gpi_dir$gpi_config $rfile  > "$outpath"
fi
if [ $gaspi_mpi_shmem = "mpi" ] ; then
    echo "RUN MPI"
    mpirun -np $ranks -f $machineFile ./../build/ra_mpi > "$outpath"
fi
if [ $gaspi_mpi_shmem = "shmem" ] ; then
    echo "RUN SHMEM"
    mpirun -np $ranks -f $machineFile ./../build/ra_shmem > "$outpath"
fi