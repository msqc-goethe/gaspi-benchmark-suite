...
int main(int argc, char **argv)
{
  ...
  if(iProc==0)printf("Running with Open MP, thread count: %i\n", omp_get_max_threads());
#endif
  ...
  *GUPs = -1;
  ...
  malloc_all(sizeof(s64Int) * NumProcs*MAXTHREADS , (void **)&updates);// An array of length npes to avoid overwrites
  ...
  *ran = starts(4*(MinLocalTableSize * iProc));
  ...
  long long *remote_val_p;
  malloc_all(sizeof(long long)*MAXTHREADS, (void **)&remote_val_p);
  BARRIER_ALL();
  // Begin timed section  

  RealTime = -RTSEC();
  //for (iterate = 0; iterate < ProcNumUpdates; iterate++)
  //ProcNumUpdates=10000;
  //int test[6]={0,0,0,0,0,0};
  omp_set_num_threads(thread_num);

#pragma omp parallel for firstprivate(ran) private(remote_proc) \
    shared(HPCC_Table, remote_val_p)
  for (iterate = 0; iterate < ProcNumUpdates; iterate++)
  {
    //printf("Threadnum: %d\n", omp_get_thread_num());
    ...
    *ran = (*ran << 1) ^ ((s64Int)*ran < ZERO64B ? POLY : ZERO64B);
    ...
    LONG_LONG_GET(&remote_val_p[omp_get_thread_num()], &HPCC_Table[*ran & (LocalTableSize - 1)], remote_proc);
    QUIET();
    *(remote_val_p+omp_get_thread_num()) ^= *ran;
    LONG_LONG_PUT(&HPCC_Table[*ran & (LocalTableSize - 1)], &remote_val_p[omp_get_thread_num()], remote_proc);
  }
  /*for (int i = 0; i < 6;i++){
    printf("Threadnumber: %d Count: %d\n ", i, test[i]);
  }*/
    BARRIER_ALL();
  // End timed section 
  RealTime += RTSEC();
