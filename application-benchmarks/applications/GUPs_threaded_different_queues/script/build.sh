...
source paths.sh
#suppress warnings
flags="-pipe -w -lm -ldl -Wall -frename-registers -pedantic -std=c99 -D_POSIX_C_SOURCE=199309L -fopenmp"

optLevel="-O3"
debugLevel="-g"
buildDir="${PROGRAM_DIR}/GUPs/build"
name="build/GUPs"
gpiDir="$2"
if [ -z $2 ] ; then
	gpiDir=$GPI_DIR
...
name="build/ra_$gaspi_mpi_shmem"
##################################
cd ../
if ! [ -d $buildDir ] ; then
	mkdir -p build
