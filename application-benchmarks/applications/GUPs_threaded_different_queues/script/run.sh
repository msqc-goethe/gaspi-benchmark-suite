...
if [ $gaspi_mpi_shmem = "gaspi" ] ; then
    $exePath -m $machineFile  -n $ranks  $runfile $gpi_dir$gpi_config $rfile $thread # > test.txt
fi
if [ $gaspi_mpi_shmem = "mpi" ] ; then
    mpirun -np $ranks -f $machineFile ./../build/ra_mpi $thread
fi
if [ $gaspi_mpi_shmem = "shmem" ] ; then
    mpirun -np $ranks -f $machineFile ./../build/ra_shmem $thread
fi