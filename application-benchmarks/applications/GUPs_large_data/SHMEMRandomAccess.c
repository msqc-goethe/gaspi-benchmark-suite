...
int main(int argc, char **argv)
{
  ...
  window_size = 50000000*sizeof(s64Int);
  ...
  segment_size = 50000000*sizeof(s64Int);
  //init GASPIS
...
#endif
#endif
  if(iProc == 0) printf("Scale Size: %d\n", data_size);
  ...
  *GUPs = -1;
  ...
  LocalTableSize = LocalTableSize / data_size;
  malloc_all(sizeof(u64Int)*LocalTableSize*data_size , (void **)&HPCC_Table);
  BARRIER_ALL();

  // Default number of global updates to table: 4x number of table entries 
  NumUpdates_Default = 4 * TableSize;
  ProcNumUpdates = 4*LocalTableSize*data_size;
  ...
  for (i = 0; i < LocalTableSize * data_size; i++)
    HPCC_Table[i] = iProc;
  ...
  *ran = starts(4*(MinLocalTableSize * iProc));
  ...
  char *remote_val;
  malloc_all(sizeof(char)*data_size*8 , (void **)&remote_val);
  BARRIER_ALL();
  // Begin timed section  
  ...
  for (iterate = 0; iterate < ProcNumUpdates; iterate++)
  {
    ...
    *ran = (*ran << 1) ^ ((s64Int)*ran < ZERO64B ? POLY : ZERO64B);
    ...
    GET_MEM(remote_val, &HPCC_Table[(*ran & (LocalTableSize - 1))*1], remote_proc);
    QUIET();
    *remote_val ^= *ran;
    PUT_MEM(&HPCC_Table[(*ran & (LocalTableSize - 1))*1], remote_val, remote_proc);
  }
  BARRIER_ALL();
  // End timed section 
  RealTime += RTSEC();
