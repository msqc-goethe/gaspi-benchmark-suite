...
static void assign_score_nb(score_matrix_t *A, index_t m, index_t n, score_t *new_value){
  int target_ep = n / A->local_length;
  int local_index = n %A->local_length;
  SHORT_PUT_NB(&(A->scores[index2d(m%3,local_index,A->local_length)]), new_value, 1, target_ep);
}

static void assign_gap_nb(score_matrix_t *A, index_t m, index_t n, score_t *new_value){
  int target_ep = n / A->local_length;
  int local_index = n % A->local_length;  
  SHORT_PUT_NB(&(A->scores[index2d(m%2,local_index,A->local_length)]), new_value, 1, target_ep);
}
...
static void collect_best_results(current_ends_t **good_ends, int max_reports, int max_threads, good_match_t *answer){
  index_t copied=0;
  int max_values=0;
  current_ends_t *current_end;
  malloc_all(sizeof(current_ends_t), (void **)&current_end);
  current_ends_t collected_ends;
  malloc_all(sizeof(score_t) * good_ends[0]->size * num_nodes, (void **)&collected_ends.goodScores);
  malloc_all(sizeof(index_t) * good_ends[0]->size * num_nodes, (void **)&collected_ends.goodEnds[0]);
  malloc_all(sizeof(index_t) * good_ends[0]->size * num_nodes,(void **)&collected_ends.goodEnds[1]);
...
good_match_t *pairwise_align(seq_data_t *seq_data, sim_matrix_t *sim_matrix, const int minScore, const int maxReports, const int minSeparation) {
  ...
  index_t* zero_seg, *buf;

  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&current_main);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&current_match);
  malloc_all(sizeof(index_t), (void **)&zero_seg);
  malloc_all(sizeof(index_t), (void **)&buf);
  malloc_all(sizeof(score_t), (void **)&main_codon);
  malloc_all(sizeof(score_t), (void **)&match_codon);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&G);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&W);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&E);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&F);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&cmp_a);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&cmp_b);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&cmp_c);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&new_score);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&next_G);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&next_F);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&next_E);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&next_match);
  malloc_all(sizeof(score_t)*omp_get_max_threads(), (void **)&next_main);
 ...
 //   printf("Time measured: %.8f seconds.int local size : %d \n", elapsed, main_seq->local_size);
    *W = sim_matrix->similarity[*main_codon][*match_codon];
    assign_score_nb(score_matrix,0,0,0 > *W ? (score_t*) zero_seg : W);
    *buf = -gapFirst + *W;
    assign_gap_nb(main_gap_matrix, 0, 0, (score_t *)buf);
    assign_gap_nb(match_gap_matrix, 0, 0, (score_t*) buf);
    //idx=1 m=0,1 n =1,0
    //fetch_from_seq(main_seq, 0, main_codon);
    ...
    *cmp_a = *cmp_a > *G ? *cmp_a : *G;
    assign_score_nb(score_matrix,1,1,cmp_a);
    *cmp_a = *E - gapExtend;
    *cmp_b = *G - gapFirst;
    assign_gap_nb(main_gap_matrix,1,0,*cmp_a > *cmp_b ? cmp_a : cmp_b);
    *buf = -gapFirst;
    assign_gap_nb(match_gap_matrix, 1, 0, -gapFirst > *cmp_b ? (score_t*) buf : cmp_b);
    ...
    *cmp_a = 0 > *F ? 0 : *F;
    *cmp_a = *cmp_a > *G ? *cmp_a : *G;
    assign_score_nb(score_matrix,1,0,cmp_a);
    *cmp_a = *F - gapExtend;
    *cmp_b = *G - gapFirst;
    *buf = -gapFirst;
    assign_gap_nb(main_gap_matrix, 1, 1, -gapFirst > *cmp_b ? (score_t*) buf : cmp_b);
    assign_gap_nb(match_gap_matrix,1,1,*cmp_a > *cmp_b ? cmp_a : cmp_b);
  ...
  for (index_t idx = 2; idx < seq_data->match->length * 2 - 1; idx++)
  {
    ...
    if(score_end < local_main_start && score_start > local_main_end){
      ...
      fetch_from_seq_nb(main_seq,local_start,next_main);
      fetch_from_seq_nb(match_seq,idx - score_start,next_match);
      fetch_score_nb(score_matrix, (idx-2)%3, local_start-1, next_G);
      fetch_gap_nb(match_gap_matrix, idx-1, idx - (score_start+1), next_F);
      fetch_gap_nb(main_gap_matrix, idx-1, local_start-1, next_E);
      QUIET();
    }
    for (int tdx = 1; tdx < omp_get_max_threads();tdx++)
    {
      *(next_main + tdx) = *next_main;
      *(next_match + tdx) = *next_match;
      *(next_G + tdx) = *next_G;
      *(next_E + tdx) = *next_E;
      *(next_F + tdx) = *next_F;
    }
...
#pragma omp parallel for \
   private(m,n) \
   shared(current_main, current_match, F, E, G, W, cmp_a, cmp_b, cmp_c, new_score,next_main,\
             next_match,next_G, next_E, next_F,good_ends,idx, local_start, local_end, main_seq,\
             match_seq,main_gap_matrix,match_gap_matrix,seq_data,score_matrix,sim_matrix) \
   schedule(static) ordered
          for(index_t antidiagonal = local_start; antidiagonal <= local_end; antidiagonal++)
      {
        ...
        n = idx - m;
        WAIT_NB();
        //printf("IDX %d Loop IDX: %d Stelle 1 rank %d thread: %d \n", idx,antidiagonal, rank, omp_get_thread_num());
        *(current_main + omp_get_thread_num()) = *(next_main + omp_get_thread_num());
        *(current_match + omp_get_thread_num()) = *(next_match + omp_get_thread_num());
        *(G + omp_get_thread_num()) = *(next_G + omp_get_thread_num());
        *(F + omp_get_thread_num()) = *(next_F + omp_get_thread_num());
        *(E + omp_get_thread_num()) = *(next_E + omp_get_thread_num());

        if (m < (seq_data->main->length - 1))
          fetch_from_seq_nb(main_seq, m + 1, (next_main + omp_get_thread_num()));
        if (n > 0)
          fetch_from_seq_nb(match_seq, n - 1, (next_match + omp_get_thread_num()));
        if (n > 1)
          fetch_gap_nb(match_gap_matrix, idx - 1, n - 2, (next_F + omp_get_thread_num()));

        fetch_gap_nb(main_gap_matrix, idx - 1, m, (next_E + omp_get_thread_num()));
        fetch_score_nb(score_matrix, (idx - 2) % 3, m, (next_G + omp_get_thread_num()));

      *(cmp_a+omp_get_thread_num()) = 0;
      *(cmp_a+omp_get_thread_num()) = *(cmp_a+omp_get_thread_num()) > *(E+omp_get_thread_num()) ? *(cmp_a+omp_get_thread_num()) : *(E+omp_get_thread_num());
      *(cmp_a+omp_get_thread_num()) = *(cmp_a+omp_get_thread_num()) > *(F+omp_get_thread_num()) ? *(cmp_a+omp_get_thread_num()) : *(F+omp_get_thread_num());
      *(W+omp_get_thread_num()) = sim_matrix->similarity[*(current_main+omp_get_thread_num())][*(current_match+omp_get_thread_num())];
      *(G+omp_get_thread_num()) += *(W+omp_get_thread_num());
      *(new_score+omp_get_thread_num()) = *(cmp_a+omp_get_thread_num()) > *(G+omp_get_thread_num()) ? *(cmp_a+omp_get_thread_num()) : *(G+omp_get_thread_num());

      if((*(new_score+omp_get_thread_num()) > good_ends[omp_get_thread_num()]->min_score && *(W+omp_get_thread_num()) > 0 && *(new_score+omp_get_thread_num()) == *(G+omp_get_thread_num()))){

        if (m+1 == seq_data->main->length || n == 0) {
          considerAdding(*(new_score+omp_get_thread_num()), minSeparation, m, n, maxReports, good_ends[omp_get_thread_num()]);
        } else {

        if((m == main_len - 1) || (n == 0) || sim_matrix->similarity[*(next_main+omp_get_thread_num())][*(next_match+omp_get_thread_num())] <= 0){
          considerAdding(*(new_score+omp_get_thread_num()), minSeparation, m, n, maxReports, good_ends[omp_get_thread_num()]);
        }
        }
      }
      *(cmp_a+omp_get_thread_num()) = *(E+omp_get_thread_num()) - gapExtend;
      *(cmp_b+omp_get_thread_num()) = *(G+omp_get_thread_num()) - gapFirst;
      *(cmp_c+omp_get_thread_num()) = *(F+omp_get_thread_num()) - gapExtend;

      
      assign_gap_nb(match_gap_matrix, idx, n, *(cmp_c+omp_get_thread_num()) > *(cmp_b+omp_get_thread_num()) ? (cmp_c+omp_get_thread_num()) : (cmp_b+omp_get_thread_num()));
      assign_score_nb(score_matrix,idx,m,(new_score+omp_get_thread_num()));
      assign_gap_nb(main_gap_matrix, idx, m, *(cmp_a+omp_get_thread_num()) > *(cmp_b+omp_get_thread_num()) ? (cmp_a+omp_get_thread_num()) : (cmp_b+omp_get_thread_num()));
