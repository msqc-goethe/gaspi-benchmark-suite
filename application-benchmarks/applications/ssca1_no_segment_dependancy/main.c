...
int main(int argc, char **argv)
{
  ...
  /*current_ends_t *current_end_ = &segment_base_ary[37030];//220
  //malloc_all(sizeof(current_ends_t), (void **)&current_end_);
  current_ends_t *good_ends_= &segment_base_ary[220];//220;
  good_ends_->min_score=10;
  good_ends_->report=20;
  good_ends_->size=30;
  BARRIER_ALL();*/
  if (rank == 0)
  {
    ...
    //GETMEM(current_end_, good_ends_, sizeof(current_ends_t), 0);
  }
/*
  codon_t main_codon  ;
  codon_t match_codon = 0;
  const seq_t *main_seq = seq_data->main;
  const seq_t *match_seq = seq_data->match;
  int number = 10;
  if (rank == 1){
  //write_to_seq(main_seq, number, 100);
  fetch_from_seq(main_seq, number, &match_codon);
  printf("\n rank:%d codon %d \n",rank,match_codon);
  }
  for (int i = 0; i < 100; i++)
  {
    fetch_from_seq(main_seq,i,&match_codon);
    //write_to_seq(main_seq, i, 5);
    printf("idx%d rank:%d codon %d \n", i,rank,match_codon);
  }*/
