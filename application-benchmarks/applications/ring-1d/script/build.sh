...
source paths.sh
#suppress warnings
flags="-pipe -Wextra -w -Wshadow -lm -ldl -Wall -frename-registers -pedantic -std=c99 -D_POSIX_C_SOURCE=199309L -fopenmp"
...
name="build/ring_$gaspi_mpi_shmem"
##################################
cd ../
if ! [ -d $buildDir ] ; then
	mkdir -p build
...
if [ $gaspi_mpi_shmem = "gaspi" ] ; then
	...
	#gcc "left_right_dataflow_halo.c" $gpiDirSharedLibs $flags -DUSE_GASPI $optLevel $debugLevel -I$gpiDirInclude -o $name
fi
