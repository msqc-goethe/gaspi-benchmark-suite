...
static void considerAdding(score_t score,
                           int minSeparation,
                           index_t main_index,
                           index_t match_index,
                           int maxReports,
                           current_ends_t* score_ends) {

	//first scan the list to see if there is a match already that is closer
	for (int idx = 0; idx < score_ends->report; idx++) {
		if (unsigned_abs_diff(score_ends->goodEnds[0][idx], main_index) <
		        minSeparation ||
		    unsigned_abs_diff(score_ends->goodEnds[1][idx], match_index) <
		        minSeparation) {
	...
	if (score_ends->report == score_ends->size) {
		...
		if (index_array == NULL)
			index_array = (index_t*) malloc(score_ends->size * sizeof(index_t));
		if (sorted_array == NULL)
			sorted_array =
			    (score_t*) malloc(score_ends->size * sizeof(score_t));
		if (best_index == NULL)
			best_index = (index_t*) malloc(score_ends->size * sizeof(index_t));

		memcpy(sorted_array,
		       score_ends->goodScores,
		       sizeof(score_t) * score_ends->report);
		index_sort(sorted_array, index_array, score_ends->report);

		worst_keeper = score_ends->size - maxReports;
		score_ends->min_score =
		    score_ends->goodScores[best_index[worst_keeper]];

		for (int index_for_index = worst_keeper;
		     index_for_index < score_ends->size;
		     index_for_index++) {
		...
		for (int idx = 0; idx < new_best_index; idx++) {
			score_ends->goodScores[idx] =
			    score_ends->goodScores[best_index[idx]];
			score_ends->goodEnds[0][idx] =
			    score_ends->goodEnds[0][best_index[idx]];
			score_ends->goodEnds[1][idx] =
			    score_ends->goodEnds[1][best_index[idx]];
		}
...
static score_matrix_t* alloc_score_matrix(index_t matrix_length) {
	score_matrix_t* new_alloc =
	    (score_matrix_t*) malloc(sizeof(score_matrix_t));
	assert(new_alloc != NULL);
	new_alloc->length = matrix_length;
	new_alloc->local_length = (matrix_length / num_nodes);

	malloc_all(sizeof(score_t) * 3 * new_alloc->local_length,
	           (void**) &new_alloc->scores);
	assert(new_alloc->scores != NULL);
	touch_memory(new_alloc->scores,
	             sizeof(score_t) * 3 * new_alloc->local_length);
	return new_alloc;
}
...
static gap_matrix_t* alloc_gap_matrix(index_t matrix_length) {
	...
	malloc_all(sizeof(score_t) * 2 * new_alloc->local_length,
	           (void**) &new_alloc->scores);
	assert(new_alloc->scores != NULL);
	touch_memory(new_alloc->scores,
	             sizeof(score_t) * 2 * new_alloc->local_length);
	return new_alloc;
}
...
static void fetch_score(score_matrix_t* A, index_t m, index_t n, score_t* in) {
	...
	SHORT_GET(in,
	          &(A->scores[index2d(m % 3, local_index, A->local_length)]),
	          1,
	          target_ep);
}
...
static void fetch_gap(gap_matrix_t* A, index_t m, index_t n, score_t* in) {
	...
	SHORT_GET(in,
	          &(A->scores[index2d(m % 2, local_index, A->local_length)]),
	          1,
	          target_ep);
}
...
static void assign_score(score_matrix_t* A,
                         index_t m,
                         index_t n,
                         score_t new_value) {
	int target_ep = n / A->local_length;
	...
	SHORT_PUT(&(A->scores[index2d(m % 3, local_index, A->local_length)]),
	          &new_value,
	          1,
	          target_ep);
}

static void assign_gap(score_matrix_t* A,
                       index_t m,
                       index_t n,
                       score_t new_value) {
	int target_ep = n / A->local_length;
	...
	SHORT_PUT(&(A->scores[index2d(m % 2, local_index, A->local_length)]),
	          &new_value,
	          1,
	          target_ep);
}
...
static void collect_best_results(current_ends_t** good_ends,
                                 int max_reports,
                                 int max_threads,
                                 good_match_t* answer) {
	index_t copied = 0;
	...
	collected_ends.goodScores =
	    malloc(sizeof(score_t) * good_ends[0]->size * num_nodes);
	collected_ends.goodEnds[0] =
	    malloc(sizeof(index_t) * good_ends[0]->size * num_nodes);
	collected_ends.goodEnds[1] =
	    malloc(sizeof(index_t) * good_ends[0]->size * num_nodes);

	for (int idx = 0; idx < num_nodes; idx++) {
		GETMEM(&current_end, good_ends[0], sizeof(current_ends_t), idx);
		SHORT_GET(&(collected_ends.goodScores[copied]),
		          good_ends[0]->goodScores,
		          current_end.report,
		          idx);
		LONG_GET(&(collected_ends.goodEnds[0][copied]),
		         good_ends[0]->goodEnds[0],
		         current_end.report,
		         idx);
		LONG_GET(&(collected_ends.goodEnds[1][copied]),
		         good_ends[0]->goodEnds[1],
		         current_end.report,
		         idx);
...
good_match_t* pairwise_align(seq_data_t* seq_data,
                             ...
                             const int minScore,
                             const int maxReports,
                             const int minSeparation) {
...
#pragma omp parallel for
	...
	for (int jdx = 0; jdx < max_threads; jdx++) {
		...
		malloc_all(sizeof(score_t) * sortReports,
		           (void**) &good_ends[idx]->goodScores);
		malloc_all(sizeof(index_t) * sortReports,
		           (void**) &good_ends[idx]->goodEnds[0]);
		malloc_all(sizeof(index_t) * sortReports,
		           (void**) &good_ends[idx]->goodEnds[1]);
		good_ends[idx]->min_score = minScore;
	}

	score_matrix_t* restrict score_matrix =
	    alloc_score_matrix(seq_data->match->length);
	gap_matrix_t* restrict main_gap_matrix =
	    alloc_gap_matrix(seq_data->match->length);
	gap_matrix_t* restrict match_gap_matrix =
	    alloc_gap_matrix(seq_data->match->length);
  ...
  */
	...
	if (rank == 0) {
		...
		assign_gap(
		    match_gap_matrix, 1, 0, -gapFirst > cmp_b ? -gapFirst : cmp_b);
		...
		cmp_b = G - gapFirst;
		assign_gap(
		    main_gap_matrix, 1, 1, -gapFirst > cmp_b ? -gapFirst : cmp_b);
	...
	for (index_t idx = 2; idx < seq_data->match->length * 2 - 1; idx++) {
		...
		score_start = idx > (seq_data->match->length - 1)
		                  ? (idx - (seq_data->match->length - 1))
		                  : 0;
		score_end = idx < (seq_data->match->length - 1)
		                ? (idx)
		                : (seq_data->match->length - 1);
		...
		if (idx < seq_data->match->length) {
			if (rank == 0) {
				...
				if ((new_score > good_ends[omp_get_thread_num()]->min_score &&
				     W > 0 && new_score == G)) {
					if (m + 1 == seq_data->main->length || n == 0) {
						considerAdding(new_score,
						               minSeparation,
						               m,
						               n,
						               maxReports,
						               good_ends[omp_get_thread_num()]);
					}
					else {
						...
						if ((m == main_len - 1) || (n == 0) ||
						    sim_matrix->similarity[next_main][next_match] <=
						        0) {
							considerAdding(new_score,
							               minSeparation,
							               m,
				...
				cmp_b = G - gapFirst;
				assign_gap(
				    match_gap_matrix, idx, n, cmp_a > cmp_b ? cmp_a : cmp_b);
				...
				cmp_b = G - gapFirst;
				assign_gap(
				    main_gap_matrix, idx, m, cmp_a > cmp_b ? cmp_a : cmp_b);
			}
			score_start++;
		...
		if (score_end < local_main_start && score_start > local_main_end) {
			...
			fetch_gap(
			    match_gap_matrix, idx - 1, idx - (score_start + 1), &next_F);
		...
		for (index_t antidiagonal = local_start; antidiagonal <= local_end;
		     antidiagonal++) {
			m = antidiagonal;
			n = idx - m;
#ifdef USE_PREFETCH
			...
			if ((new_score > good_ends[omp_get_thread_num()]->min_score &&
				...
				if (m + 1 == seq_data->main->length || n == 0) {
					...
					if ((m == main_len - 1) || (n == 0) ||
					    sim_matrix->similarity[next_main][next_match] <= 0) {
						considerAdding(new_score,
						               minSeparation,
						               m,
						               n,
