...
int verify_alignment(good_match_t* A, int maxDisplay) {
	...
	if (A->bestLength == 0) {
		...
		printf("\nFound %lu acceptable alignments with scores from %i to %i.\n",
		       A->bestLength,
		       (int) A->bestScores[0],
		       (int) A->bestScores[A->bestLength - 1]);
		if (maxDisplay > 0) {
	...
	for (int m = 0; m < A->bestLength; m++) {
		...
		if (score != A->bestScores[m]) {
			retval = 1;
			printf(
			    "\nverifyAlignment %i failed; Kernel 1 reported %i versus "
			    "Kernel 2 reported %i:\n---------------------------\n",
			    m,
			    (int) A->bestScores[m],
			    (int) score);
		...
		if (m < maxDisplay) {
			...
			memset(
			    match_codon_chain, '\0', A->bestSeqs[m].match->length * 3 + 1);

			assemble_acid_chain(A,
			                    main_acid_chain,
			                    A->bestSeqs[m].main,
			                    A->bestSeqs[m].main->length);
			assemble_acid_chain(A,
			                    match_acid_chain,
			                    A->bestSeqs[m].match,
			                    A->bestSeqs[m].match->length);

			assemble_codon_chain(A,
			                     main_codon_chain,
			                     A->bestSeqs[m].main,
			                     A->bestSeqs[m].main->length);
			assemble_codon_chain(A,
...
void tracepath(good_match_t* A,
  ...
  */
	...
	if (dir == 0 ||
	    (dir &
	     T[index2d(
	         i, j, sizeT)])) // if we aren't skipping and it isn't part of a gap
	...
	if (dir & 4) {
		tracepath(
		    A, sizeT, T, ei, ej, i - 1, j - 1, 0, rs, sequence, depth + 1);
		if (rs[0] != -1) {
...
void doScan(good_match_t* A, int* bestR, int minSeparation, int report) {
	...
	int gapFirst =
	    A->simMatrix->gapStart +
	    A->simMatrix->gapExtend; // total penalty for first codon in gap
	...
	int sizeT = 2 * (A->simMatrix->matchLimit > A->seqData->max_validation
	                     ? A->simMatrix->matchLimit
	                     : A->seqData->max_validation);
		...
		while (dj <= lj && di > 0 && e < sizeT && f < sizeT) {
			...
			if (s == goal) {
				...
				for (int r = 0; r < *bestR; r++) {
					...
					if ((compare_a > compare_b ? compare_a : compare_b) <
					    minSeparation) {
				...
				if (rs[0] == -1 &&
				    rs[1] ==
				        -1) // tracepath returns -1 in rs if there is no path
				{
				...
				/* changed sizeof(seq_t) to sizeof(seq_data_t)
		 * to address warnin: "memcpy’ reading 32 bytes from a region of size 24"
		 */
			...
			T[index2d(e, f, sizeT)] =
			    T[index2d(e, f, sizeT)] + (E[e] == s) + 2 * (F[f] == s);
