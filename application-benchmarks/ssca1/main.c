...
void display_elapsed(struct timeval* start_time) {
	...
	printf(
	    "\n\tElapsed time: %li hour(s), %li minute(s), %li second(s), %li "
	    "milliseconds,  %li micro second(s).\n",
	    hours_elapsed,
	    minutes_elapsed,
	    seconds_elapsed,
	    miliseconds_elapsed,
...
int main(int argc, char** argv) {
	...
	if (rank == 0)
		printf("Running with GASPI, ranks = %i\n", num_nodes);

	gaspi_segment_size = 1000000;
	setup_gaspi_segment(gaspi_segment_size);
...
#ifdef USE_PREFETCH
	if (rank == 0)
		printf("Using OpenSHMEM prefetching\n");
#else
	if (rank == 0)
		printf("Disabling OpenSHMEM prefetching\n");
#endif
	...
	if (global_parameters.ENABLE_VERIF || global_parameters.CONSTANT_RNG) {
		...
		random_seed = (unsigned int) time(
		    NULL); /* casting from time_t to unsigned int we can lose precision... no big deal here */
	...
	if (rank == 0) {
		printf(
		    "HPCS SSCA #1 Bioinformatics Sequence Alignment Executable "
		    "Specification:\nRunning...\n");

		printf("Using seed %u\n", random_seed);

		printf(
		    "\nScalable Data Generator - genScalData() beginning "
		    "execution...\n");
	}
  ...
  */
	...
	sim_matrix = gen_sim_matrix(global_parameters.SIM_EXACT,
	                            global_parameters.SIM_SIMILAR,
	                            global_parameters.SIM_DISSIMILAR,
	                            global_parameters.GAP_START,
	                            global_parameters.GAP_EXTEND,
	                            global_parameters.MATCH_LIMIT);

	seq_data = gen_scal_data(sim_matrix,
	...
	QUIET();
#endif
	A = pairwise_align(seq_data,
	                   sim_matrix,
	                   global_parameters.K1_MIN_SCORE,
	                   global_parameters.K1_MAX_REPORTS,
	                   global_parameters.K1_MIN_SEPARATION);
	...
	if (rank == 0) {
		...
		scanBackward(A,
		             global_parameters.K2_MAX_REPORTS,
		             global_parameters.K2_MIN_SEPARATION);
