...
char validations[2][3][32] = {{"ACDEFG*IDENTICAL*HIKLMN",
...
index_t* gen_indexes(int num_indexes,
                     index_t rand_max,
                     index_t min_seperation) {
	int not_done;
	...
	do {
		...
		for (int idx = 0; idx < num_indexes; idx++) {
			for (int jdx = idx + 1; jdx < num_indexes; jdx++) {
				if (indexes[idx] - indexes[jdx] < min_seperation)
					not_done = 1;
			}
		}
	} while (not_done);
...
void create_sequence(seq_t* sequence,
                     char validations[][32],
                     int num_validations,
                     sim_matrix_t* simMatrix) {
	...
	if (rank == 0) {
		for (int idx = 0; idx < num_validations; idx++) {
			...
			for (int jdx = 0; jdx < end; jdx++) {
				write_to_seq(sequence,
				             indexes[idx] + jdx,
				             simMatrix->encode[(int) validations[idx][jdx]]);
			}
		}
	}
	free(indexes);
...
#pragma omp parallel for num_threads(thread_number)
	...
	for (int idx = 0; idx < 2; idx++) {
		touch_memory(gen_sequences[idx]->sequence,
		             sizeof(codon_t) * gen_sequences[idx]->backing_memory);
