...
#else
#ifdef USE_GASPI
#define SHORT_GET(target, source, num_elems, rank)	gaspi_read(segment_id,((void *)target - (void *)segment_base_ary),rank,segment_id,((void *)source - (void *)segment_base_ary),num_elems * sizeof(short), queue,timeout); QUIET()
#else
...
#else
#ifdef USE_GASPI
#define SHORT_GET_NB(target, source, num_elems, rank) gaspi_read(segment_id, ((void *)target - (void *)segment_base_ary), rank, segment_id, ((void *)source - (void *)segment_base_ary), num_elems * sizeof(uint16_t), queue, timeout);
#else
...
#else
#ifdef USE_GASPI
#define LONG_GET(target, source, num_elems, rank)	gaspi_read(segment_id, (void *)target - (void *)segment_base_ary, rank, segment_id, (void *)source - (void *)segment_base_ary, num_elems * sizeof(index_t),queue, timeout); QUIET()
#else
...
#else
#ifdef USE_GASPI
#define GETMEM(target, source, length, rank)		gaspi_read(segment_id, (void *)target - segment_base_ary, rank,segment_id,(void *)source - segment_base_ary, length, queue, timeout); QUIET()
#else
...
#else
#ifdef USE_GASPI
#define SHORT_PUT(target, source, num_elems, rank)	gaspi_write(segment_id, (void*)source -segment_base_ary, rank, segment_id, (void*)target -segment_base_ary, num_elems * sizeof(short), queue, timeout); QUIET()
#else
...
static inline void fetch_from_seq(const seq_t *in, index_t const codon_index, codon_t *out){
  ...
  //printf("\n fetch from Sequence; my rank %d; dest rank: %d ", rank, target_ep);
