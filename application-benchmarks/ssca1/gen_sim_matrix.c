...
static const char* l_similarity[] =
...
static const char* r_similarity[] =
    {"R", "cgt", "cgc", "cga", "cgg", "aga", "agg"};
static const char* s_similarity[] =
...
static const char** similarity[] = {
    a_similarity,   c_similarity, d_similarity, e_similarity, f_similarity,
    g_similarity,   h_similarity, i_similarity, k_similarity, l_similarity,
    m_similarity,   n_similarity, p_similarity, q_similarity, r_similarity,
    s_similarity,   t_similarity, v_similarity, w_similarity, y_similarity,
    star_similarity};

/* lengths for the similarity matrix above, */
static const int similarity_length[] = {5, 3, 3, 3, 3, 5, 3, 4, 3, 7, 2,
                                        3, 5, 3, 7, 7, 5, 5, 2, 3, 4};
...
sim_matrix_t* gen_sim_matrix(int exact,
                             int similar,
                             int dissimilar,
                             int gapStart,
                             int gapExtend,
	...
	char process_acid;
	int ccode = 0;
	sim_matrix_t* the_similarity_matrix =
	    (sim_matrix_t*) malloc(sizeof(sim_matrix_t));
	...
	for (int encode_index = 0; encode_index < 128; encode_index++)
		the_similarity_matrix->encode[encode_index] = STAR;
	/* 21 rows in similiarity matrix */
	for (int similarity_matrix_index = 0; similarity_matrix_index < 21;
	     similarity_matrix_index++) {
		process_acid = similarity[similarity_matrix_index][0][0];
		for (int codon_index = 1;
		     codon_index < similarity_length[similarity_matrix_index];
		     codon_index++) {
			ccode = 0;
			for (int base_index = 0; base_index < 3; base_index++) {
				switch (similarity[similarity_matrix_index][codon_index]
				                  [base_index]) {
					case 'a':
						ccode = 0 + 4 * ccode;
						break;
					...
					default:
						fprintf(stderr,
						        "Wandered into territory I wasn't supposed to, "
						        "aborting.\n");
						abort();
						break;
				}
			}
			strcpy(the_similarity_matrix->codon[ccode],
			       similarity[similarity_matrix_index][codon_index]);
	...
	for (int i = 0; i < 64; i++) {
		for (int j = 0; j < 64; j++) {
			...
			else if (the_similarity_matrix->aminoAcid[i] ==
			         the_similarity_matrix->aminoAcid[j]) {
