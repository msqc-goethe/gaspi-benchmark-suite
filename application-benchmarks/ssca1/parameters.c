...
void init_parameters(parameters_t* parameters) {
   ...
   */
	...
	parameters->MAIN_SEQ_LENGTH =
	    ceil(pow(2.0, (scale / 2.0))); /* Total main codon sequence length. */
	parameters->MATCH_SEQ_LENGTH =
	    ceil(pow(2.0, (scale / 2.0))); /* Total match codon sequence length. */
   ...
   */
	...
	parameters->K1_MAX_REPORTS =
	    200; /* >0  Maximum end-points reported to K2 */

	/* Kernel 2 */
	parameters->K2_MIN_SEPARATION =
	    parameters->K1_MIN_SEPARATION; /* >=0 Minimum start-point separation */
	parameters->K2_MAX_REPORTS =
	    ceil(parameters->K1_MAX_REPORTS /
	         2); /* >0 Maximum sequences reported to K3 */
	parameters->K2_DISPLAY = 10; /* >=0 Number of reports to display */

	/* Kernel 3 */
	parameters->K3_MIN_SCORE = 10; /* >0 Minimum end-point score */
	parameters->K3_MIN_SEPARATION =
	    parameters->K1_MIN_SEPARATION; /* >=0 Minimum end-point separation */
	...
	if (parameters->SIM_EXACT <= 0 || parameters->SIM_DISSIMILAR >= 0 ||
	    parameters->GAP_START < 0 || parameters->GAP_EXTEND <= 0) {
		fprintf(stderr,
		        "Similarity parameters set in getUserParameters are invalid\n");
		abort();
	}

	if (parameters->MISMATCH_PENALTY <= 0 || parameters->SPACE_PENALTY <= 0) {
		fprintf(stderr,
		        "Difference parameters set in getUserParameters are invalid\n");
		abort();
	}

	if (parameters->K1_MAX_REPORTS <= 0) {
		fprintf(stderr,
		        "Kernel 1 parameters set in getUserParameters are invalid\n");
		abort();
	}

	if (parameters->K2_MAX_REPORTS <= 0) {
		fprintf(stderr,
		        "Kernel 2 parameters set in getUserParameters are invalid\n");
		abort();
	}

	if (parameters->K3_MAX_REPORTS <= 0) {
		fprintf(stderr,
		        "Kernel 3 parameters set in getUserParameters are invalid\n");
		abort();
	}
}
