 ...
 */
  ...
  */
  ...
  fetch_from_seq(A->seqData->main,ei-i,scan_back_main);
  Ci = *scan_back_main;
  fetch_from_seq(A->seqData->match, ej - j, scan_back_main);
  Cj = *scan_back_main;
  if (dir & 4)
  {
...
void doScan(good_match_t *A, int *bestR, int minSeparation, int report) {
  ...
  int rs[2];
  codon_t *main_codon;
  main_codon= scan_back_main;
  codon_t *match_codon;
  match_codon = scan_back_match;
