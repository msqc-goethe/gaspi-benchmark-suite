...
static void fetch_gap(gap_matrix_t *A, index_t m, index_t n, score_t *in){
  int target_ep = n / A->local_length;
  int local_index = n %A->local_length;
  //printf("\nA length: %d   score: %d\n", A->local_length,A->scores[index2d(m % 2, local_index, A->local_length)]);
...
#pragma omp parallel for
  ...
  //  while(gogogo==0){}
  //}
   // fetch_from_seq(match_seq,55, &match_codon);
   // printf("\nMain tester  main codon %d match codon %d\n", main_codon, match_codon);
  ...
  //printf("\nHello Diana main codon %d match codon %d\n", main_codon, match_codon);
  ...
  cmp_a = E - gapExtend;
  cmp_b = G - gapFirst;
  //printf("\n 390 W: %d   G: %d   cmp_a: %d   cmp_b: %d   \n", W, G, cmp_a, cmp_b);
  //printf("\nE: %d   gapExtend: %d   gapFirst: %d \n", E, gapExtend, gapFirst);
  ...
  if (idx < seq_data->match->length)
  {
    if(rank == 0){
      ...
      //printf("\n IDx%d  main codon %d match codon %d n: %d \n", idx,main_codon, match_codon,n);
      W = sim_matrix->similarity[main_codon][match_codon];
      G = W;
      ...
      new_score= cmp_a;
 // printf("\n IDx%d W: %d   G: %d   cmp_a: %d   cmp_b: %d   ",idx, W, G, cmp_a, cmp_b);
 // printf("E: %d   gapExtend: %d   gapFirst: %d \n", E, gapExtend, gapFirst);
    ...
    if(score_end < local_main_start && score_start > local_main_end){
      ...
      //printf("\n IDx%d next_main: %d   next_G: %d   next_F: %d   next_E: %d   maxReports: %d   \n",idx, next_main, next_G, next_F, next_E,maxReports);
