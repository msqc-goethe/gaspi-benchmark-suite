...
void handle_slice(slice* sl,
	...
	if (sl->index == left_halo + 1) {
		if (my_stage > right_stage ||
		    !test_or_die(LEFT(iProc, nProc),
		                 segment_id,
		                 left_data_available[old_buffer_id],
		                 1)) {
	...
	if (sl->index == right_halo - 1) {
		if (my_stage > left_stage ||
		    !test_or_die(RIGHT(iProc, nProc),
		                 segment_id,
		                 right_data_available[old_buffer_id],
		                 1)) {
			return;
		}
	}
