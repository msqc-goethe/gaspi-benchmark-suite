...
void data_compute_lower(double* array,
                        int buffer_to,
                        int buffer_from,
                        int slice_id,
                        int VLEN) {
	...
	for (int j = 0; j < VLEN; ++j) {
		array_ELEM_lower(buffer_to, slice_id, j) =
		    array_ELEM_lower(buffer_from, slice_id + 1, j);
	}
}

void data_compute_upper(double* array,
                        int buffer_to,
                        int buffer_from,
                        int slice_id,
                        int VLEN) {
	...
	for (int j = 0; j < VLEN; ++j) {
		array_ELEM_upper(buffer_to, slice_id, j) =
		    array_ELEM_upper(buffer_from, slice_id - 1, j);
	}
}

void data_compute(double* array,
                  int buffer_to,
                  int buffer_from,
                  int slice_id,
                  int VLEN) {
